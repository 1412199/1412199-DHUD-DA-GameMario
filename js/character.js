...
Character.prototype.update = function(){	
	...
	if(b = this.map.colllide(this.left+2,vbottom,false,this.canEat) || this.map.colllide(this.right-2,vbottom,false,this.canEat)) // bottom
	{		
		if(b && b!=true)
		{
	...
	{			
		b = this.map.colllide(this.left+2,vtop,this.canDestroyObstacles,this.canEat);
		var b1 = this.map.colllide(this.right-2,vtop,this.canDestroyObstacles,this.canEat);
		if(!b)
			b = b1;
	...
	b = null;
	// detect side collisions with the obstacles	
	if(this.speedX<0 && this.left<0 || (b = this.map.colllide(vleft,vtop+6,false,this.canEat) ||
		this.map.colllide(vleft,vbottom-4,false,this.canEat))) // left
	{
		if(b && b!=true)
			this.left = b.right;
		// reverse the moving direction if it is auto moving	
		this.speedX = this.isAutoMoving? this.speed: 0;	
			
	}else if(this.speedX>0 && this.right>=this.map.width || (b = this.map.colllide(vright,vtop+6,false,this.canEat) ||
			this.map.colllide(vright,vbottom-4,false,this.canEat)))
	{			
		if(b && b!=true)
